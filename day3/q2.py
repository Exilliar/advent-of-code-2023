...
def checkAround(x, y, grid):
    ...
    if x != 0 and grid[y][x-1] in numbers:
        toReturn.append(getNumber(x-1, y, grid, maxX))
    # x-1, y-1
    if x != 0 and y != 0 and grid[y-1][x-1] in numbers:
        bottom1 = True
        toReturn.append(getNumber(x-1, y-1, grid, maxX))
    # x, y-1
    if y != 0 and bottom1 != True and grid[y-1][x] in numbers:
        bottom2 = True
        toReturn.append(getNumber(x, y-1, grid, maxX))
    # x+1, y-1
...
def getNumber(x, y, grid, maxX): # MAKE SURE THIS RETURNS AN INT
    gridLine = grid[y]
